...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRapidzAmount"))
        settings.setValue("nAnonymizeRapidzAmount", 1000);
    nAnonymizeRapidzAmount = settings.value("nAnonymizeRapidzAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeRapidzAmount"))
        SoftSetArg("-anonymizerapidzamount", settings.value("nAnonymizeRapidzAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeRapidzAmount:
            return QVariant(nAnonymizeRapidzAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeRapidzAmount:
            nAnonymizeRapidzAmount = value.toInt();
            settings.setValue("nAnonymizeRapidzAmount", nAnonymizeRapidzAmount);
            emit AnonymizeRapidzAmountChanged(nAnonymizeRapidzAmount);
            break;
        default:
            break;
        }
