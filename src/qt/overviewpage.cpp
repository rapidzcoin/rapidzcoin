...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeRapidzAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeRapidzAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeRapidzAmount = strAnonymizeRapidzAmount.remove(strAnonymizeRapidzAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRapidzAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeRapidzAmount*COIN) nMaxToAnonymize = nAnonymizeRapidzAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeRapidzAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeRapidzAmount));
        strAnonymizeRapidzAmount = strAnonymizeRapidzAmount.remove(strAnonymizeRapidzAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRapidzAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
