...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: rapidz.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: rapidzd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizerapidzamount=<n> " + _("Keep N Rapidz anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Rapidz is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Rapidz is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Rapidz version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeRapidzAmount = GetArg("-anonymizerapidzamount", 0);
    if(nAnonymizeRapidzAmount > 999999) nAnonymizeRapidzAmount = 999999;
    if(nAnonymizeRapidzAmount < 2) nAnonymizeRapidzAmount = 2;
    ...
    LogPrintf("Anonymize Rapidz Amount %d\n", nAnonymizeRapidzAmount);
